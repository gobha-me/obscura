...
// scorch mark in a room, a door log, a body's last position — and the vocabulary
// the rest of the game reasons in. An Evidence item is a *claim about the
// world*, complete and true; it is not what the player has seen. Hiding is
// redaction.hpp's job, and keeping the two apart is what lets the solver check a
// case against the full set while the renderer draws only part of it.
//
// Everything here must be a pure function of (Hull, Roster, Incident). If a
// derivation needs a coin flip, it takes a Seed argument — it does not reach for
// one.
...
[[nodiscard]] auto derive(const Hull& hull, const Roster& roster, const Incident& incident) -> EvidenceSet;
...
[[nodiscard]] auto is_consistent(const Evidence& item, const Roster& roster, const Incident& incident) -> bool;
