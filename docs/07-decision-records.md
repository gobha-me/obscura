...
| 08 | Where does RtAudio live? | Inside termforge it is convenient and breaks the stdlib-only guarantee. Outside costs one more CMake project. | **Decided:** OBSCURA owns `audio::Sink` and `audio::NullSink`; a real RtAudio backend remains a separate optional M2 target. TermForge stays stdlib-only. |
...
| 10 | Public GitHub now, or private Gitea first? | Public early gets the audience and free scrutiny of the determinism claims. Private early lets M0 fail quietly, which matters when its gate is "does this feel good". | **Completed:** private through the M0 gate, public from M1. GitHub issues are the canonical work record; stable T-series identifiers preserve design intent without duplicated ticket bodies. |
