...
//   * GCC (13+) additionally predefines __SANITIZE_ADDRESS__ /
//   __SANITIZE_THREAD__.
//   * GCC has NO standard UBSan predefine before __has_feature (GCC 14+), so on
//     GCC <= 13 the UBSan branch relies on -DOBSCURA_UBSAN from
//     undefined.cmake.
...
#define SMOKE_TSAN 1
#endif
#if __has_feature(undefined_behavior_sanitizer) || defined(OBSCURA_UBSAN)
...
TEST_CASE("ASan detects heap-buffer-overflow", "[smoke][asan]") {
  ...
  volatile std::size_t i =
      3;     // volatile defeats compile-time OOB elision (survives -O3)
